...
jQuery(document).ready(function ($) {
  ...
  $(document).on('click', '.service', function (e) {
    ...
    if (
      document.querySelector('.row1expand') !== null &&
      $(this).hasClass('selectedservice')
    ) {
      /* Collapse if expanded and same click */
      $('#servicechoicetext').animate(
        { left: '390px', opacity: '0.4' },
        '200',
        servicechoiceblank
      );
      function servicechoiceblank() {
        document.getElementById('servicechoicetext').innerHTML =
          'View Services';
      }
      ...
      function bodytext() {
        ...
        if (servicechoicetextvar.trim() === 'DELIVERABLE-BASED SERVICES') {
          document.getElementById('deliverableservices').style.display =
            'block';
        }
      ...
      if (
        ...
        $('#servicechoicetext').animate(
          { left: '350px', opacity: '0.9' },
          '400',
          servicechoicechange
        );
        $('#servicechoicetext').animate(
          { left: '70px', opacity: '0.9' },
          '700'
        );
      }
    } else {
